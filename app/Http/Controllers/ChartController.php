...
class ChartController extends Controller
{
    ...
    public function index()
    {
        ...
        $plug = Plug::select('id', 'type')
            ->where([['status', 1], ['is_check', 1], ['is_new', 1]])
            ->get();
...
//    $sysinfo = GetWMI($wmi,"Win32_OperatingSystem", array('LastBootUpTime','TotalVisibleMemorySize','FreePhysicalMemory','Caption','CSDVersion','SerialNumber','InstallDate'));
    ...
    public function userCreated(Request $request)
    {
        ...
        $s = '';
        $s = '';
        if (isset($request->time[0]) && isset($request->time[1])) {
        ...
        $res = User::select('created_at')
            ->when($request->time != '' && isset($request->time[0]), function ($query) use ($e, $s) {
    ...
    public function login(Request $request)
    {
        ...
        $res = DB::table('login')->when($request->time != '' && isset($request->time[0]), function ($query) use ($e, $s) {
