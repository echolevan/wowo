...
<template>
    <div>
        ...
        <tr>
            ...
            <td width="32%"><span id="stime">{{info.sysinfo.time}}</span></td>
            <td width="20%">服务器运行时间</td>
            <td width="30%" colspan="3"><span id="uptime" v-html="info.sysinfo.uptime"></span></td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td colspan="5">
                物理内存：共
                <font color="#CC0000">{{ (info.sysinfo.memTotal / 1024).toFixed(3) }}  </font>G
                , 已用
                <font color="#CC0000"><span id="UsedMemory">{{ (info.sysinfo.memUsed / 1024).toFixed(3) }} </span></font>G
                , 空闲
                ...
                真实内存使用
                <span id="memRealUsed">{{(info.sysinfo.memRealUsed / 1024).toFixed(3)}} </span>G
                , 真实内存空闲
                <span id="memRealFree">{{(info.sysinfo.memRealFree / 1024).toFixed(3)}} </span>G
                , 使用率
                <span id="memRealPercent " class="use_class" style="color: #CC0000">{{info.sysinfo.memRealPercent}}</span>
                %
